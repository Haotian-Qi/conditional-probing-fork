...
class ProbeRegimen(InitYAMLObject):
    ...
    def __init__(
        ...
        self.max_gradient_steps = (
            sys.maxsize if max_gradient_steps == -1 else max_gradient_steps
        )
    ...
    def train_until_convergence(
        ...
        if finetune:
            self.optimizer = optim.Adam(
                list(probe.parameters()) + list(model.parameters()),
                lr=0.00001,
                weight_decay=0,
            )
        else:
        ...
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            self.optimizer, mode="min", factor=0.5, patience=0
        )
            ...
            for batch in tqdm(train_dataset, desc="[training]"):
                ...
                if gradient_steps % eval_dev_every == 0:
                    ...
                    if epoch_dev_loss / epoch_dev_loss_count < min_dev_loss - 0.001:
                        ...
                        tqdm.write(
                            "Saving probe parameters to {}".format(self.params_path)
                        )
