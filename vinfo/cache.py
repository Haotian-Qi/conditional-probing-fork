...
class WholeDatasetCache(InitYAMLObject):
    ...
    def get_cache_path_and_check(self, split, task_name):
        ...
        if split == TRAIN_STR:
            ...
            return (
                self.train_path + ".cache." + task_name + ".hdf5",
                read_cache,
                write_cache,
            )
        elif split == DEV_STR:
            ...
            return (
                self.dev_path + ".cache." + task_name + ".hdf5",
                read_cache,
                write_cache,
            )
        elif split == TEST_STR:
            ...
            return (
                self.test_path + ".cache." + task_name + ".hdf5",
                read_cache,
                write_cache,
            )
        else:
        ...
        for cache_path in glob.glob(path + "*cache." + task_name + ".*hdf5"):
            ...
            if os.path.exists(lock_path):
                return (
                    False,
                    False,
                )  # Cache locked, being written to by object in this process
